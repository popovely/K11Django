# 장고에서의 views.py파일은 컨트롤러의 역할
'''
단축함수 (django.shortcuts)
    : 장고에서 웹 프로그램 개발시 자주 사용하는 기능들을 제공하는 내장함수.
      render()함수 등이 있다.
'''
...
get_object_or_404(모델클래스, 검색조건)
    : 모델클래스(테이블)에서 검색조건에 맞는 객체를 조회하여 반환한다.
      만약 조건에 맞는 객체가 없다면 Http404에러를 발생시킨다.
'''
def detail(request, question_id):
    # URL패턴을 통해 전달된 일련번호로 Question테이블을 검색후 반환
    question = get_object_or_404(Question, pk=question_id)
    # 조회한 데이터를 Dictionary를 통해 템플릿으로 반환
    return render(request, 'livepolls/detail.html', {'question':question})

# 투표처리 : 선택한 항목의 vote컬럼을 +1한다.
def vote(request, question_id):
    ...
    # 예외발생시 detail페이지를 다시 보여준다. 이 때 question항목과 에러메세지를 전달.
    except (KeyError, Choice.DoesNotExist):
        return render(request, 'livepolls/detail.html', {
            'qeustion':question,
            'error_message':"You didn't select a choice.",
        })
    else:   # try절의 코드가 정상실행되면 가져온 레코드를 통해 votes항목을 +1 한다.
        selected_choice.votes += 1
        # save()를 통해 앞에서 처리한 내용을 실제 테이블에 적용. (업데이트 된다.)
        selected_choice.save()
        # 정상처리되면 results 페이지로 이동
