...
테이블의 컬럼은 클래스의 변수로 매핑한다.
django.db.models.Model 클래스를 상속받아 정의하고,
각 클래스 변수의 타입고 장고에서 미리 정의해둔 필드 클래스를 사용한다.
'''
...
class Question(models.Model):
    ...
    '''
    객체를 문자열로 표현할 때 사용.
    해당 함수를 정의하지 않으면 테이블명이 제대로 표시되지 않는다.
    '''
    def __str__(self):
...
class Choice(models.Model):
    ...
    FK로 지정된 컬럼은 자동으로 _id라는 접미사가 붙는다.
    '''
