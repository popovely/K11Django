...
'''
해당 URLconf는 프로젝트 레벨이므로
livepolls앱의 views.py(모듈)을 임포트해야만 접근할 수 있다.
'''
...
장고에서의 URL패턴 작성법
 ...
 ⇀ 별칭은 필수사항이 아니지만 만들어두면 차후에 유용하게 사용가능.
'''
...
urlpatterns = [
    ...
    # http://127.0.0.1:8000/
    path('', views.main, name='main'), # 로켓화면 대신 메인화면으로 대체할 예정이므로 추가
    
    # 방법1 : 1개의 파일에 작성
    # http://127.0.0.1:8000/admin/livepolls/
...
'''
    ...
    하지만 방법2와 같이 파일을 2개로 나누면
    하나의 패턴을 위한 네임스페이스가 생성되는 개념이므로,
    한번만 수정하면 돼서 편리하다.
