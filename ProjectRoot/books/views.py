...
from django.views.generic.base import TemplateView  # 템플릿뷰
...
class BooksModelView(TemplateView): # 해당클래스는 TemplateView를 상속하여 정의
    # TemplateView를 사용하면 해당 클래스변수를 오버라이딩 해야한다.
    template_name = 'books/index.html'  # 템플릿 파일경로 지정
...
'''
제네릭 뷰 중 DetailView를 사용하여 상세페이지 구현
    : DetailView를 상속하는 경우, 특정 레코드 하나를 조회한 후
      context변수에 저장한다.
      URLConf에서 지정한 파라미터를 통해 특정 레코드 하나를 조회한 후 템플릿으로 전달하게 된다.
'''
class BookDetail(DetailView):
#   [자동으로 처리되는 부분]
    # 1. 테이블로부터 조회한 값은 object라는 변수명으로 context에 저장한다.
    # 2. "모델명소문자_detail.html" 이라는 템플릿 파일명을 찾아 렌더링한다.
    model = Book
